...
class DeveloperTools:
    ...
    # Hello world command
    @commands.command(name="hello", brief="A simple Hello World! command.", usage="")
    ...
    # Exception testing
    @commands.command(name="except", brief="Throw a text exception.", usage="")
    async def test_exception(self, ctx):
        raise UserWarning("Testing an exception!")

    # Change bot status
    @commands.command(name="cbs", brief="Change bot status. Developer command.", usage="string", hidden=True)
    ...
    # Get system uptime
    @commands.command(name="system_uptime", aliases=["suptime"], brief="Checks the system /proc/uptime.", usage="", hidden=True)
    ...
    # Botsay
    @commands.command(name="botsay", aliases=["bs"], usage="<string>", hidden=True)
    ...
    async def nick_user(self, ctx, *args):
        if not (ctx.author.id == 276531286443556865):
            return

        await ctx.message.mentions[0].edit(nick=str(args[1]))
